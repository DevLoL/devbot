...
def get_connection():
    ...
    bitly = bitly_api.Connection(access_token='3eed5b80216f14d7519f44d19ca7b0b6390d7d12')
    return bitly
...
def setup(bot=None):
    global url_finder, exclusion_char, bitly
...
def title_auto(bot, trigger):
    ...
    for title, domain, short in results[:4]:
        message = '[ %s ] - (%s) - %s' % (title, domain, short)
...
def process_urls(bot, trigger, urls):
    ...
    for url in urls:
        if not url.startswith(exclusion_char):
            ...
            if matched:
                continue
            # See if the url is long and shorten it
            if len(url) > 30:
            ...
            if title:
                results.append((title, getTLD(url), short))
    return results
